...
CREATE TABLE public."MoldingMachinesStates" (
    ...
    time_stamp timestamp without time zone DEFAULT now(),
    recipe_state jsonb DEFAULT '{}'::jsonb
);
...
CREATE VIEW public.latestmoldingmachinesstates AS
 SELECT DISTINCT ON (mms.machine_serial_number) mms.machine_serial_number,
    mms.is_running,
    mms.is_stopped,
    mms.is_automatic,
    mms.is_manual,
    ...
    mms.recipe_state,
    COALESCE(mmc.cycles_per_second, (0)::numeric) AS cycles_per_second
   FROM (public."MoldingMachinesStates" mms
     LEFT JOIN ( SELECT "MoldingMachinesCycles".machine_serial_number,
            ((count(*))::numeric / EXTRACT(epoch FROM '00:00:01'::interval)) AS cycles_per_second
           FROM public."MoldingMachinesCycles"
          WHERE ("MoldingMachinesCycles".time_stamp > (now() - '01:00:00'::interval))
          GROUP BY "MoldingMachinesCycles".machine_serial_number) mmc ON (((mms.machine_serial_number)::text = (mmc.machine_serial_number)::text)));
...
CREATE VIEW public.machine_recipe_cycle_counts AS
 SELECT result.machine_name,
    result.recipe_counts
   FROM ( SELECT machines.machine_serial_number AS machine_name,
            ( SELECT json_object_agg(subquery.mold_recipe_name, subquery.recipe_count) AS json_object_agg
                   FROM ( SELECT m.mold_recipe_name,
                            count(*) AS recipe_count
                           FROM public."MoldingMachinesCycles" m
                          WHERE ((m.time_stamp > COALESCE(( SELECT max(maintenance.time_stamp) AS max
                                   FROM public."MoldingMachinesCycles" maintenance
                                  WHERE ((maintenance.is_maintenance = true) AND ((maintenance.machine_serial_number)::text = (machines.machine_serial_number)::text))), '-infinity'::timestamp without time zone)) AND ((m.machine_serial_number)::text = (machines.machine_serial_number)::text))
                          GROUP BY m.mold_recipe_name
                         HAVING (count(*) > 0)) subquery) AS recipe_counts
           FROM ( SELECT DISTINCT "MoldingMachinesCycles".machine_serial_number
                   FROM public."MoldingMachinesCycles") machines) result
  WHERE (result.recipe_counts IS NOT NULL);
 ...
 SELECT m.mold_id,
   ...
   FROM (public."Molds" m
     JOIN ( SELECT DISTINCT ON ("MoldingMachinesStates".last_recipe_name) "MoldingMachinesStates".last_recipe_name
          ...
          WHERE ("MoldingMachinesStates".time_stamp >= (now() - '168:00:00'::interval))) subquery ON (((m.mold_recipe_name)::text = (subquery.last_recipe_name)::text)));
...
1	P01230	35 lbs nov.xml	41930	84936	t	2023-11-15 12:59:36.475548
10	P01286	Mixto 25 (KGS).xml	87941	95824	t	2023-11-15 13:03:59.848866
5	P01229	10 lbs octubre.xml	30036	90124	t	2023-11-15 13:02:13.945593
\.
...
COPY public."MoldingMachinesStates" (state_id, machine_serial_number, is_running, is_stopped, is_automatic, is_manual, is_disconnected, last_recipe_name, life_cycles, mold_distance, current_cycle_time, last_cycle_time, working_time_auto, working_time_manual, working_time_total, working_time_hydraulic, alarm_00_description, time_stamp, recipe_state) FROM stdin;
